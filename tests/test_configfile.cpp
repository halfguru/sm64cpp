...
TEST_CASE("Configfile ConfigManager direct load/save", "[configfile]")
{
    const char *test_ini = "test_direct_manager.ini";

    // Modify target values
    settings::Settings::get().configWindowWidth = 1440;
    settings::Settings::get().configWindowHeight = 900;

    config::ConfigManager::save(test_ini, settings::Settings::get());

    // Mutate state
    settings::Settings::get().configWindowWidth = 640;

    config::ConfigManager::load(test_ini, settings::Settings::get());

    REQUIRE(settings::Settings::get().configWindowWidth == 1440);
    REQUIRE(settings::Settings::get().configWindowHeight == 900);
